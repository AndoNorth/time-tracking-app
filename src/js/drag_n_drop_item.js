...
addGlobalEventListener("dragstart", ".list-item", (e) => {
  ...
  if (list == doingList.id) {
    // if picked from doing list do...
    ...
    if (isValueUndefined(timeStamps) || timeStamps.length == 0) {
      return;
    }
    ...
    if (timeDiff > MIN_TIME_IN_LIST) {
      ...
      console.log(`time stamp removed`);
    }
...
addGlobalEventListener("dragend", ".list-item", (e) => {
  ...
  if (list == doingList.id) {
    // if dropped into doing list do...
    let timeStamps = listItem["time-stamps"]; // current timeStamps
    //console.log(`timeStampsBefore:${JSON.stringify([...timeStamps])}`); // DEBUG POINT
    // if timeStamps is undefined or empty, set to [new timeStamp]
    if (
      isValueUndefined(timeStamps) ||
      isValueUndefined(timeStamps[0]) ||
      timeStamps.length == 0
    ) {
...
function getItemAfterHeldItem(list, y) {
  ...
  return draggableListItems.reduce(
    (closest, listItem) => {
  ...
  ) /* initialise offset with max negative number */
